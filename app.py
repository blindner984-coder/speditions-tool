    ...
    try:
        ...
        def get_sur(name, keys, f_teu=False, p_teu=False):
            ...
            for m in re.finditer(regex, text, re.I):
                ...
                if re.search(r'\b(?:not subject to|incl|ind|included)\b', block[:60], re.I): continue
                p_match = re.search(r'(?<!\d)(\d{1,4}(?:[.,]\d{1,2})?)\s*(EUR|USD)', block, re.I)
                if p_match:
                    val = float(p_match[1].replace(',', '.'))
                    curr = p_match[2].upper()
                    if f_teu or (not p_teu and 'teu' in block.lower()): val *= 2
                    return f"{name} = {val:.2f} {curr}"
            return None

        # ECA Logik: 47 EUR TEU -> 94 EUR für 40'HC
        ...
        if "via pol" in text.lower():
            for block in re.split(r'Port\s+of\s+Discharge', text, flags=re.I)[1:]:
                ...
                for r in re.finditer(r'via\s+POL\s+([A-Za-z/]+)\s+[\d.,]+\s*[A-Z]{3}\s+([\d.,]+)\s*([A-Z]{3})', block, re.I):
                    res_list.append({'Carrier': 'MSC', 'Contract Number': contract_no, 'Port of Loading': r.group(1), 'Port of Destination': pod, 'Valid from': v_from, 'Valid to': v_to, '40HC': float(r.group(2).replace(',', '.')), 'Currency': r.group(3).upper(), 'Included Prepaid Surcharges 40HC': prep_str})
        else:
            pod_m = re.search(r'Port\s+of\s+Discharge[\s\n]*([A-Za-z\s\-]+)', text, re.I)
            pod = re.sub(r'(?i)Volume|DV|HC|Freetime|Remarks', '', pod_m.group(1)).strip().split()[0].title() if pod_m else "Unbekannt"
            rate_m = re.search(r'(\d{3,4}(?:[.,]\d{1,2})?)\s*(USD|EUR)', text[text.find("40'"):text.find("40'")+120])
            if rate_m:
                res_list.append({'Carrier': 'MSC', 'Contract Number': contract_no, 'Port of Loading': 'Hamburg', 'Port of Destination': pod, 'Valid from': v_from, 'Valid to': v_to, '40HC': float(rate_m.group(1).replace(',', '.')), 'Currency': rate_m.group(2).upper(), 'Included Prepaid Surcharges 40HC': prep_str})
...
with tab_suche:
    ...
    if not data:
        ...
        mask = (pd.to_datetime(df['Valid from dt']) <= datum) & (pd.to_datetime(df['Valid to dt']) >= datum)
...
with tab_upload:
    uploaded = st.file_uploader("PDFs hochladen", type="pdf", accept_multiple_files=True)
